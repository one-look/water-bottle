...
def _build_rag_prompt(
    ...
    """Formats retrieved context documents, conversation history, and user query into a single structured prompt.

    Args:
        query: Raw user query string.
        documents: List of retrieved text chunks from vector search.
        history: List of prior message turns for this session.
...
async def generate_rag_response(
    ...
    x_session_id: str = Header(
        ..., alias="X-Session-ID", description="Session Identifier"
    ),
) -> RAGResponse:
    """Orchestrates query embedding, multi-tenant vector retrieval, async memory context, and LLM text generation."""
    ...
    try:
        ...
        # Step 0: Instantiate memory store via factory and fetch session history asynchronously
        logger.info("Instantiating memory service via MemoryFactory...")
        memory_store = MemoryFactory.create(config)
        history = await memory_store.get_history(x_session_id)
        ...
        # Step 5: Save interaction turns to conversation history asynchronously
        await memory_store.add_message(
            x_session_id, role="user", content=request.query
        )
        await memory_store.add_message(
            x_session_id, role="assistant", content=generated_answer
        )
